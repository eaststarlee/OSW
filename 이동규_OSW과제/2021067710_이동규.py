...
def main():
    ...
    pygame.display.set_caption('2021067710이동규')

    showTextScreen('MY TETRIS')
    while True: # game loop
        if random.randint(0, 1) == 0:
            pygame.mixer.music.load('Hover.mp3') #tetrisb.mid
        else:
...
def runGame():
    ...
    movingDown = False # nodte: there is no movingUp variable
    movingLeft = False
    movingRight = False
    score = 0
...
def drawStatus(score, level, passed_time):
    ...
    # draw the time text
    passedTimeSurf = BASICFONT.render('Time: %.2f' % passed_time, True, TEXTCOLOR)  # 경과 시간 표시
